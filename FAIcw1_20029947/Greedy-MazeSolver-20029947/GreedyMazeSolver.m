...
% traversing the map and store the coordinate of OBSTACLE, start node and OBSTACLE
for x = 1 : MAX_X
    for y = 1 : MAX_Y
...
while(xNode ~= xTarget || yNode ~= yTarget)
    ...
    for i = 1 : exp_count
        ...
        for j = 1 : QUEUE_COUNT
            ...
            if(exp(i, 1) == QUEUE(j, 2) && exp(i, 2) == QUEUE(j, 3))
                QUEUE(j, 6) = min(QUEUE(j, 6), exp(i, 3)); % QUEUE(j, 7) and exp(i, 4) are h(n)
                if QUEUE(j, 6) == exp(i, 3)
                    % update parents, h(n)
                    QUEUE(j, 4) = xNode; % parent node
                    QUEUE(j, 5) = yNode; % parent node
    ...
    % the value 5 stands for all the routes that Greedy has processed
    maze(xNode, yNode) = 5;
