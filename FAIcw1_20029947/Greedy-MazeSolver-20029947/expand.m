...
function exp_array = expand(node_x, node_y, gn, xTarget, yTarget, OBSTACLE, MAX_X, MAX_Y)
    ...
    for k = 1 : -1 : -1 % explore surrounding locations
        for j = 1 : -1 : -1
            if (k ~= j && k + j ~= 0) % the children node can only be above, below, left and right to the parent node
                s_x = node_x + k;
                s_y = node_y + j;
                if( (s_x > 0 && s_x <= MAX_X) && (s_y > 0 && s_y <= MAX_Y)) % node within array bound
                    flag = 1;                    
                    for c1 = 1 : c2
                        if(s_x == OBSTACLE(c1, 1) && s_y == OBSTACLE(c1, 2)) % the expanded node can't be on obstacle
                            flag = 0;
                        end
