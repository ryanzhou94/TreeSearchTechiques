...
% show the beginning of optimal path from end side
if ((xval == xTarget + 1) && (yval == yTarget)) % to check if temp <= 0
    maze(xval, yval) = 6; % set the end point in black
    dispMaze(maze);
end
...
while(parent_x ~= xStart || parent_y ~= yStart)
    ...
    parent_x = QUEUE(inode, 4); % find the parent
